 ...
 *                       sys_hook
 * ****************************************************************/
PUBLIC  int sys_hook(int _unused1, int pos, int func, struct proc * p_proc) {
	// void *p = va2la(proc2pid(p_proc), (void*)func);
	void *p = 0x34d0;
	__asm__ __volatile__ ("xchg %bx, %bx");

	// __asm__ __volatile__ ("push %%eax; push %%ebx; push %%ecx; push %%edx; call %0"::"m"(p));
	int* address = get_table();
	address[pos] = (int*)p;
	__asm__ __volatile__ ("xchg %bx, %bx");
	//__asm__ __volatile__ ("push %%eax;push %%ebx; push %%ecx; push %%edx; call %0"::"m"(address));
}
/*****************************************************************************
 *				  ldt_seg_linear
 *****************************************************************************/
