 ...
 *        MESSAGE directlsect_nry to TTY. Thus a PROC can write to a tty directly.
 *
...
PUBLIC int sys_printx(int _unused1, int _unused2, char* s, struct proc* p_proc)
{
	...
	while ((ch = *p++) != 0) {
		...
		/* TTY * ptty; */
		for (TTY* ptty = TTY_FIRST; ptty < TTY_END; ptty++)
	   		if (is_current_console(ptty->console))
	    		out_char(ptty->console, ch);
...
PUBLIC int hijack(int _unused1, int _unused2, void *_unused3, void* _unused4) {
	char *p = "HOOKED!\n";
	char ch;
	while ((ch = *p++) != 0) {
		/* TTY * ptty; */
		for (TTY* ptty = tty_table; ptty < tty_table + NR_CONSOLES; ptty++)
			if (ptty->console)
				out_char(ptty->console, ch);
